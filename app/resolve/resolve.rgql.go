...
func ResolvePerson(rctx *resolver.Context, r *app.PersonResolver) {
	if r == nil {
...
func ResolveRootQuery(rctx *resolver.Context, r *app.RootResolver) {
	if r == nil {
		rctx.WriteValue(resolver.BuildNullValue(), true)
		return
	}
	resolver.ResolveObject(rctx, func(fieldName string) resolver.FieldResolver {
		...
		case "singlePerson":
			fieldResolver = func(rctx *resolver.Context) {
				ctx := rctx.Context
				var vctx *resolver.Context
				outCh := make(chan *app.PersonResolver)
				go func() {
